/* chromatic_aberration_stage impl. */
...
me_status_t chromatic_aberration_kernel(
        task::TaskContext&,
    ...
    p.red_dx  = static_cast<int>(read_int64_prop(props, "red_dx",  0));
    p.red_dy  = static_cast<int>(read_int64_prop(props, "red_dy",  0));
    p.blue_dx = static_cast<int>(read_int64_prop(props, "blue_dx", 0));
    p.blue_dy = static_cast<int>(read_int64_prop(props, "blue_dy", 0));
    ...
    const me_status_t s = apply_chromatic_aberration_inplace(
...
void register_chromatic_aberration_kind() {
    task::KindInfo info{
        .kind           = task::TaskKindId::RenderChromaticAberration,
        .affinity       = task::Affinity::Cpu,
        .latency        = task::Latency::Short,
        .time_invariant = true,
        .kernel         = chromatic_aberration_kernel,
        .input_schema   = {
            {"in", graph::TypeId::RgbaFrame},
        },
        .output_schema  = { {"out", graph::TypeId::RgbaFrame} },
        .param_schema   = {
            {.name = "red_dx",  .type = graph::TypeId::Int64},
            {.name = "red_dy",  .type = graph::TypeId::Int64},
            {.name = "blue_dx", .type = graph::TypeId::Int64},
            {.name = "blue_dy", .type = graph::TypeId::Int64},
        },
    };
