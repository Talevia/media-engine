...
me_status_t motion_blur_kernel(task::TaskContext&,
    ...
    p.dx_px   = static_cast<int>(read_int64_prop(props, "dx_px",   0));
    p.dy_px   = static_cast<int>(read_int64_prop(props, "dy_px",   0));
    p.samples = static_cast<int>(read_int64_prop(props, "samples", 1));
    ...
    const me_status_t s = apply_motion_blur_inplace(
...
void register_motion_blur_kind() {
    task::KindInfo info{
        .kind           = task::TaskKindId::RenderMotionBlur,
        ...
        .param_schema   = {
            {.name = "dx_px",   .type = graph::TypeId::Int64},
            {.name = "dy_px",   .type = graph::TypeId::Int64},
            {.name = "samples", .type = graph::TypeId::Int64},
        },
    };
