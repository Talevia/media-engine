...
| OpenFX SDK 1.5.1 (FetchContent: AcademySoftwareFoundation/openfx) | BSD-3 | Phase M13 (host plugin support) — CMake opt-in behind `ME_WITH_OFX` (default OFF). Headers-only consumption (`SOURCE_SUBDIR include` skips the OFX repo's CMakeLists.txt which expects Conan-provided `opengl_system` for sample-plugin builds we don't need). Provides the C-ABI types (`OfxStatus`, `OfxImageEffectHandle`, `OfxPropertySet`) + protocol constants (`kOfxStatOK`, `kOfxImageEffectActionRender`, etc.) consumed by the planned `src/openfx/` host suite implementation (see docs/OPENFX.md for the design + four-piece split into `m13-openfx-{plugin-cache,image-effect-suite,render-smoke}-impl`). The OFX standard itself is BSD-3 (LGPL-clean); per-plugin licenses VARY and the host enforces a whitelist at load time, mirroring the M11 ML model-license pattern. |
