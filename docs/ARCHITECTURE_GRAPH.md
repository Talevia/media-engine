...
```
    ...
    timeline::Timeline (IR)
              ...
              ▼
              ┌─────── orchestrator/ ───────┐
              │ Player(Timeline)            │  ← 播放会话，A/V sync (c)
              │ Exporter(Timeline)          │  ← 批编码，throughput-first (b)
              │ compose_frame_at(...)       │  ← 单帧自由函数 (a)
              │ compose_png_at(...)         │  ← 单帧 PNG 自由函数 (a)
              │                             │
...
用于 **preview / thumbnail / frame-cache / `me_render_frame`**——"给定 time，产一帧"的场景。

- 入口是 `src/orchestrator/compose_frame.{hpp,cpp}` 里的自由函数：`resolve_active_clip_at(tl, t)` → `compile_frame_graph(uri, source_t)` → `scheduler.evaluate_port(g, term, ctx{time})`。`compose_frame_at` 是 resolve+compile+await 的便利包装；`compose_png_at` 在它上面再加 sws scale + libavcodec PNG encode。
- 没有 orchestrator 类——这条路径**没有跨调用状态**，每次调用从零开始，包成类是过度设计。M5 之前曾有 `Previewer` / `CompositionThumbnailer` 类，已删；调用方（`me_render_frame` / `Player::producer_loop` / `compose_png_at`）直接调自由函数。
...
- Player（`src/orchestrator/player.cpp`）持 `(current_time, rate, is_playing)` 状态；起 Producer + Pacer 两条线程：Producer 用 (a) 路径（`resolve_active_clip_at` + `compile_frame_graph` + `scheduler.evaluate_port`，inline 而非 `compose_frame_at` 便利包装——Player 需要保留 in-flight Future 给 seek 取消）每帧取 video，用 (b) 路径调 `AudioMixer::pull_next_mixed_frame` 拉混音 chunk；Pacer 按 master clock 决定何时调 callback。
...
主实现在 `src/orchestrator/player.cpp` + `playback_clock.cpp` + `video_frame_ring.cpp`；audio chunk 直接从 producer thread 经 throttle 后送 callback（无队列，详见 commit b78d106 commit body）；C API 在 `include/media_engine/player.h`。
...
越界（例如想给路径 (a) 的自由函数挂 encoder state、或想让 (b) 自己决定播放节拍）→ 停下来重想；不是 path 选错就是 feature 设计错。
...
// (a) per-frame —— 自由函数，没有类。
me_status_t resolve_active_clip_at(const me::Timeline&, me_rational_t time, ResolvedClip*);
std::pair<graph::Graph, graph::PortRef>
    compile_frame_graph(const std::string& uri, me_rational_t source_t);
me_status_t compose_frame_at(me_engine_t*, const me::Timeline&, me_rational_t time,
                              std::shared_ptr<graph::RgbaFrameData>* out, std::string* err);
me_status_t compose_png_at(me_engine_t*, const me::Timeline&, me_rational_t time,
                            int max_w, int max_h,
                            uint8_t** out_png, size_t* out_size, std::string* err);
...
(b) 与 (c) 都接收 Timeline；(a) 直接吃 `const Timeline&`——它没有跨调用状态、不需要 ctor。Player 与 (a) 共用 `compile_frame_graph` 但状态模型不同：(a) 无状态、调用即返一帧；Player 有状态（current_time / rate / is_playing）、长寿命、对外承担时钟。
...
路径 (a) 的自由函数（`compose_frame_at` / `compose_png_at`）类似——`resolve_active_clip_at(t)` → `compile_frame_graph(uri, source_t)` → `evaluate_port(g, video_terminal, {.time=source_t})`。
...
| `src/api/render.cpp` | `me_render_start` → `Exporter(timeline).export_to(...)`；`me_render_frame` → `resolve_active_clip_at` + DiskCache peek + `compose_frame_at` + me_frame wrap | shipped |
| `src/orchestrator/compose_frame.cpp` | 路径 (a) 入口：`resolve_active_clip_at` / `compile_frame_graph` / `compose_frame_at` / `compose_png_at`。`scheduler.cache` 接管 in-process 重复访问；`disk_cache` 持久层由 `me_render_frame` 包装层负责 | shipped |
