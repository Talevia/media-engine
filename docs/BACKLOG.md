...
- **transform-animated-support** — Transform 层 + parse + compose + e2e 全部落地（layers 1-3 + e2e tripwire via `animated-transform-e2e` cycle; `test_compose_sink_e2e` "animated transform (translateX keyframes) renders end-to-end" 产物 406KB）。**剩余工作：audio-gain 动画化**——`me::Clip::gain_db` 从 `std::optional<double>` 迁到 `std::optional<AnimatedNumber>`；AudioMixer 需要 per-sample-window `evaluate_at(T)` —— mixer 抽象当前不接 T 参数，这个迁移要改 mixer API（track_feed 的 `gain_linear` 从 ctor-time 常量换成 per-pull eval）+ 测试覆盖。可考虑独立 bullet `gain-db-animated-support`。Milestone §M3，Rubric §5.1。
