...
- **debt-reencode-pipeline-split-process-segment** — `src/orchestrator/reencode_pipeline.cpp` FIFO 下沉之后还 523 行（scan-debt 2026-04-23 snapshot），主要体量在 `process_segment()`（~250 行的 per-clip 开 decoder / seek / read-loop / 段末 flush）。本身就是一个自成关注点的流程，应该和 `SharedEncState` 一起搬到 `process_segment.cpp/hpp` 让 `reencode_pipeline.cpp` 降回 orchestration（open mux + open encoder + for-each segment + flush encoder + write trailer）。**方向：** 新 `src/orchestrator/reencode_segment.{cpp,hpp}` 容纳 `SharedEncState` struct + `process_segment()`；`reencode_pipeline.cpp` 只留顶层流程。Milestone §M1-debt，Rubric §5.2。
- **me-timeline-loader-multi-track-reject** — `src/timeline/timeline_loader.cpp` 解析 timeline JSON 时接受 `tracks: [...]` 是任意长度 array 但 phase-1 orchestrator（Exporter）只遍历第一条 track。用户传两条 track 时第二条被**静默丢掉**，没有任何 error。**方向：** loader 侧显式 reject `tracks.size() != 1` 并给明确 err（"phase-1 passthrough + reencode support single track only; multi-track lands with multi-track-video-compose"）。等 M2 compose 真支持多轨时再 lift。Milestone §M1-debt，Rubric §5.1。
