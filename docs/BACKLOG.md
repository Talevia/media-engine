...
- **composition-thumbnail-impl** — `src/orchestrator/composition_thumbnailer.cpp:7` `STUB: composition-thumbnail-impl`；`composition_thumbnailer.cpp:13` 返回 `ME_E_UNSUPPORTED`. 仓库最后一个实质 stub (scan-debt.sh: "Marked stubs: 1")。M2 feature 没实装，导致 timeline 级 composited thumbnail API 不可用。**方向：** 基于 me_render_frame 路径（Previewer::frame_at 已实装）聚合同一 T 的多轨 compose 结果输出 PNG；复用 `me_thumbnail_png` 的 PNG 编码 helper。Milestone §M2-debt (cross)，Rubric §5.3。
- **debt-split-timeline-impl-hpp** — `src/timeline/timeline_impl.hpp` 433 行（阈值 400）。近期 cycle 30 的 TrackKind::Subtitle + SubtitleClipParams 再加码。**方向：** 拆成 `timeline_ir_core.hpp`（Timeline/Track/Clip/ClipType/TrackKind 基础）+ `timeline_ir_effect.hpp`（EffectSpec 变体 + AnimatedNumber 跨引）+ `timeline_ir_synth_params.hpp`（TextClipParams / SubtitleClipParams）；`timeline_impl.hpp` 降为薄 include-all。Milestone §M7-debt (cross)，Rubric §5.3。
- **debt-split-timeline-loader-cpp** — `src/timeline/timeline_loader.cpp` 413 行（阈值 400）。合成 clip 分支 (`track_kind == me::TrackKind::Text / Subtitle`) 扩展后更拥挤。**方向：** 按 schema 区段拆 `timeline_loader_assets.cpp`（资产/颜色空间）+ `timeline_loader_tracks.cpp`（轨道 + clip 遍历）+ `timeline_loader_transitions.cpp`（过渡 adjacency）；保持 `timeline_loader.cpp` 作为 entry point 调度。Milestone §M7-debt (cross)，Rubric §5.3。
