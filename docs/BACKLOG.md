...
- **timeline-schema-animated-color-docs** — `docs/TIMELINE_SCHEMA.md` 没 AnimatedColor 文档段（`grep -n 'AnimatedColor\|animated-color' docs/TIMELINE_SCHEMA.md` 空；textParams 从 line 148 起文档齐全但 color 字段仍描述为"CSS-like string"。cycle 40 落实装实现三种 JSON shape 但 schema doc 滞后。**方向：** 在 textParams 段里把 color 字段 upgrade 为三种 JSON shape 描述（plain-string、`{static: ...}`、`{keyframes: [{t, v, interp, cp?}]}`）+ 示例 + 与 animated_number 行为共享点。Milestone §M7-debt (cross)，Rubric §5.6。
