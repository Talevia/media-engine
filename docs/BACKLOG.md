...
- **compose-transform-affine-wire** — M2 exit criterion "Transform (静态) 端到端生效（translate/scale/rotate/opacity）" 的剩余 3 轴。opacity 已在 `compose-transform-wire-opacity` cycle 落地（ComposeSink::process 读 `Clip::transform->opacity` 传给 `alpha_over`；`tests/test_compose_sink_e2e.cpp` 新增 "per-clip transform opacity renders" case）。剩余：translate / scale / rotate 是 2D spatial affine，需要 pre-composite 阶段对每 track 的 RGBA frame 按 transform matrix 变换到 dst canvas。**方向：** 新 `me::compose::affine_blit(dst, src, src_w, src_h, dst_w, dst_h, matrix)` 辅助（bilinear sampling + bounds 裁剪 + 边界 alpha=0 填充）；或用 `sws_getContext` + `sws_scale` 的 filter-with-warp 配置。compose loop per-active-track 先 `frame_to_rgba8` → `affine_blit` 到 canvas-size RGBA buffer → `alpha_over` into dst_rgba。anchor_x/y 纳入 matrix 的 origin-shift。kernel-level test（known translate/scale/rotate 数学值）+ e2e test（非 identity 变换后产物文件不同）。关闭 M2 "Transform (静态)" exit criterion。Milestone §M2，Rubric §5.1。
