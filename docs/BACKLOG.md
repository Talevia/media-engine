...
- **audio-mix-scheduler-wire** — M2 exit criterion "2+ audio tracks 混音, 带 peak limiter" 的最后两块（scheduler + sink）。Prereq 全部就位：kernel (`me::audio::mix_samples` / `peak_limiter` / `db_to_linear`)、resample wrapper (`resample_to`, 9 case / 177 assertion)、per-track feed (`AudioTrackFeed`, 8 case / 1288 assertion)、**AudioMixer class** (`src/audio/mixer.{hpp,cpp}`, 6 case / 98764 assertion，由 `audio-mix-scheduler` cycle 添加——N feed → 1 mixed AVFrame stream, per-channel mix_samples + peak_limiter, FIFO-backed frame_size 对齐, partial-drain silence padding)。剩余：(1) 重构 H264AacSink（或新 AudioCompSink）audio path 改成调 AudioMixer 而非从单 demux passthrough；需要 ComposeSink / reencode_audio 的 audio 路径分流（`src/orchestrator/reencode_audio.cpp` / `src/orchestrator/compose_sink.cpp`）。(2) Exporter audio-only track gate 翻（`src/orchestrator/exporter.cpp` 的 `return ME_E_UNSUPPORTED "standalone audio tracks not yet implemented"`）。(3) 2-track e2e determinism 测试（mono 50Hz + mono 100Hz → stereo 48k）。Milestone §M2，Rubric §5.1。
