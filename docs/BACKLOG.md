...
- **effect-registry-api-skeleton** — M3 exit criteria "EffectChain 能把连续 ≥ 2 个像素级 effect 合并成单 pass" + "≥ 3 个 GPU effect" 的 API 起步：`grep -rn 'me_effect\|class Effect' include src` 返回空——无 Effect 抽象 / registry / typed 参数 schema。VISION §3.2 硬规 typed params（不许 `Map<String, Float>`）。**方向：** 立 C API `me_effect_kind_t` enum 占位 + per-kind typed `me_<kind>_params_t` struct（例如 `me_blur_params_t { me_rational_t radius; bool horizontal_only; }`）+ 内部 `me::effect::Effect` interface + `EffectChain` 持 `vector<Effect>`（不实装任何 effect，只立架子 + 1 个 no-op identity effect 验证端到端）。Registry-by-tagged-union 而非 string-keyed。后续 GPU effect（blur / color-correct / LUT）作独立 bullet 填实装。Milestone §M3，Rubric §5.1 / §5.2。
