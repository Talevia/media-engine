...
- **multi-track-compose-sink-wire** — M2 exit criterion "2+ video tracks 叠加, alpha + blend mode" 的最后实装。三件前置已就位：schema/IR（`multi-track-video-compose` cycle）+ alpha_over 内核（`multi-track-compose-kernel` cycle，`src/compose/alpha_over.{hpp,cpp}`）+ 多轨活动 clip 解析器（`multi-track-active-clips-resolver` cycle，`src/compose/active_clips.{hpp,cpp}`）。剩余工作是把三者拼起来。`src/orchestrator/exporter.cpp:60` 的多轨 gate 仍然 `ME_E_UNSUPPORTED`。**方向：** 新 `ComposeSink`（or 扩展 `H264AacSink`）：per output frame 调 `me::compose::active_clips_at(tl, t)` → 对每个 TrackActive 从对应 demux 抽 frame at source_time → sws-scale 到 RGBA8 → `me::compose::alpha_over` 按 tracks declaration order 叠（bottom → top）→ RGBA → YUV → encoder。翻 Exporter 的 multi-track gate；加 e2e 测试（2-track timeline 跑完且两次产出字节确定）。Milestone §M2，Rubric §5.1。
