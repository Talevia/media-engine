...
- **audio-mix-scheduler-wire** — M2 exit criterion "2+ audio tracks 混音, 带 peak limiter" 的最后两块（scheduler + sink）。Prereq 全部就位：kernel、resample wrapper、per-track feed、AudioMixer class、以及 **timeline-driven builder** (`build_audio_mixer_for_timeline` in `src/audio/mixer.{hpp,cpp}`, 10 case / 99801 assertion，由 `audio-mix-builder` cycle 添加——从 Timeline + `demux_by_clip_idx` 自动筛 audio clips, 应用 `clip.gain_db → db_to_linear`, 调用 open_audio_track_feed, add_track 到 mixer)。剩余：(1) 重构 ComposeSink / H264AacSink audio path 改成调 `build_audio_mixer_for_timeline` 生成 mixer → pull mixed frames → feed AAC encoder，替代现有 "passthrough from demuxes[0]" 路径（`src/orchestrator/reencode_audio.cpp` + `src/orchestrator/compose_sink.cpp:106` "bottom track 的 clips 用作 audio source"）。(2) Exporter audio-only track gate 翻（`src/orchestrator/exporter.cpp` 的 `return ME_E_UNSUPPORTED "standalone audio tracks not yet implemented"`）。(3) 2-track e2e determinism 测试（mono 50Hz + mono 100Hz → stereo 48k）。Milestone §M2，Rubric §5.1。
