...
- **transform-animated-support** — M3 exit criterion "所有 animated property 类型的插值正确（linear / bezier / hold / stepped）" 的 3 层工作。已完成层 1（IR primitive + 插值数学 + 单测：`src/timeline/animated_number.{hpp,cpp}`, 11 case / 34 assertion, `animated-number-primitive`）+ 层 2（JSON → AnimatedNumber loader: `me::timeline_loader_detail::parse_animated_number` in `src/timeline/loader_helpers.{hpp,cpp}`, 13 case / 27 assertion, `animated-number-loader`——支持 `{"static"}` 和 `{"keyframes":[...]}` 形式，validation 覆盖 sorted-by-t / no-dup / bezier cp 约束 / 未知 key 拒）。**剩余 layer 3：Caller 迁移**。(1) `me::Clip::gain_db` 从 `std::optional<double>` 换成 `std::optional<me::AnimatedNumber>`；AudioMixer 相关路径 (mixer.cpp / track_feed.cpp) 把 `db_to_linear(*clip.gain_db)` 改成 `db_to_linear(clip.gain_db->evaluate_at(T))` —— T 需要传进 mixer，这触发 mixer API 小改。(2) `me::Transform` 的 8 `double` 字段换成 `AnimatedNumber`；`src/orchestrator/compose_sink.cpp` 和 `compose_transition_step.cpp` 的 caller 先把 8 字段 `.evaluate_at(T)` 算成 double，再调 `compose_inverse_affine` kernel（kernel 签名不变）。(3) `src/timeline/loader_helpers.cpp:parse_transform` 改调 `parse_animated_number` 代替 `parse_animated_static_number`。(4) 新 e2e 测试：timeline 带 `{"keyframes":[...]}` transform 或 gainDb，render 验证插值生效。Milestone §M3，Rubric §5.1。
