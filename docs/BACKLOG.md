...
- **debt-jni-render-cancel-test** — `grep -rn 'nativeRenderCancel\|RenderJob.cancel\|\.cancel(' bindings/jni/example/` 返回空——cycle 74 的 `Run.java` 跑全程 wait 没 cancel；cycle 75/76 各自跑 version + thumbnail，也都不触 cancel。`bindings/jni/me_jni.cpp` 有 nativeRenderCancel 但 zero JVM 端调用。一次 cancel 路径回归（如忘了 detach JNI thread on cancel）只在 talevia 集成时炸。**方向：** 新 `Cancel.java` 在 example/ — render_start → 100 ms 后 cancel + waitFor + 验 rc == ME_E_CANCELLED；CMake 加 `add_test(NAME jni_cancel_smoke ...)`。Milestone §M7-debt，Rubric §5.5。
...
- **debt-split-thumbnail-cpp** — `wc -l src/api/thumbnail.cpp` = 341；`grep -n 'me_status_t me_thumbnail' src/api/thumbnail.cpp` 显示 me_thumbnail_png + 两个 helper 都在一个 TU。文件目前混 (a) decode-frame helpers (b) PNG encode (c) JPEG encode (d) public C 入口。下一个 thumbnail 格式（webp/avif）会撞 §1a 阈值。**方向：** 抽 `src/api/thumbnail_encode_png.cpp` + `thumbnail_encode_jpeg.cpp`（若有），公共 me_thumbnail_png 入口留主文件 + 一个 detail 工厂；预计降到 ~200 行。Milestone §M7-debt (cross)，Rubric §5.3。
- **debt-split-audio-mixer-cpp** — `wc -l src/audio/mixer.cpp` = 336；近 §1a 阈值。`grep -n '^[a-zA-Z].*::\|^void \|^static ' src/audio/mixer.cpp` 看 mix loop + per-track gain 应用 + resample + AAC frame slicing 都在一个 TU。下一个 mixer feature（pan / per-channel gain / sidechain）会推过线。**方向：** 抽 `src/audio/mixer_track_feed.cpp` 装 per-track gain + resample；mixer.cpp 留 mix loop + frame slicing。Milestone §M4-debt (cross)，Rubric §5.3。
- **debt-bindings-kn-cinterop-ctest** — `grep -rn 'add_test.*gradle\|add_test.*kotlin' tests/` 返回空。cycle 27 K/N example builds via Gradle, but no ctest invokes it. Gradle only runs by hand → cinterop drift（如 `media_engine.def` 漏 declare 新 header）只在 host 集成时 surface。**方向：** ctest 加 `find_program(GRADLE_EXECUTABLE gradle)` + 找到则 `add_test(NAME kn_cinterop_smoke COMMAND gradle -p bindings/kotlin-native/example runDebugExecutableNative)`，TIMEOUT 90s（gradle bootstrap 慢）；找不到 silent-skip。Milestone §M7-debt，Rubric §5.5。
