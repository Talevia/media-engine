...
- **debt-reencode-pipeline-internal-strcmp-migration** — Cycle 48 (`debt-typed-codec-options-internal-migration`) landed the typed-codec resolver at the dispatch layer (output_sink / hevc_sw_sink / compose_sink / audio_only_sink). Sink-internal strcmp ladders remain in deeper layers: `src/orchestrator/reencode_pipeline.cpp:49-57` rejects unsupported video_codec via `opts.video_codec != "h264" && != "hevc" && != "hevc-sw"`, `src/orchestrator/reencode_video.cpp:34-54` switches on `video_codec == "hevc" / "hevc-sw"` to pick encoder name + pix_fmt + default bitrate. These consume `ReencodeOptions.video_codec` (a std::string), not `me_output_spec_t` directly. **方向：** add `me_video_codec_t video_codec_enum / me_audio_codec_t audio_codec_enum` to `ReencodeOptions`; populate from `resolve_codec_selection` at the make_output_sink dispatch layer; replace the strcmp ladders in reencode_pipeline + reencode_video with enum switches. Existing string fields remain as the diagnostic source for "unsupported codec '<name>'" error messages. Milestone §M7-debt (cross)，Rubric §5.2。
