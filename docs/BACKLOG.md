...
- **previewer-multi-track-compose-graph** — Previewer (`src/orchestrator/previewer.cpp:33-38`) 现在走 graph 框架但只挑 bottom track 单 clip，注释 `// Phase-1 single-track frame server` 标了多轨待办。多轨合成的 RenderComposeCpu / RenderCrossDissolve kernel 在 `src/task/task_kind.hpp:39-40` 占位但**无 body** (`grep -rn 'RenderComposeCpu\|RenderCrossDissolve' src/{io,compose,render} 2>/dev/null` 空)。**方向：** 注册 `RenderComposeCpu` (n inputs RgbaFrame → 1 output RgbaFrame，body 复用 `me::compose::alpha_over`) + 在 `previewer_graph.cpp:compile_frame_graph` 的多轨场景下 build N 个 decode 链 + 1 个 compose node。注：`src/orchestrator/compose_decode_loop.cpp` 的 streaming 多轨逻辑已存在但属于 Exporter 路径，不能直接搬到 per-frame；要按"每帧一个 graph"重写。Milestone §M6-debt (cross)，Rubric §5.1。
