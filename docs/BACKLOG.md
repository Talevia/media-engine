...
- **kotlin-native-cinterop-def** — M7 exit criterion "Kotlin/Native cinterop `.def` + 示例 Gradle 项目". `grep -rn 'cinterop\|\.def$' include integration bindings 2>/dev/null` 空（没有 bindings 目录；`integration` 也不存在）。**方向：** (1) 新 `bindings/kotlin-native/media_engine.def` — Kotlin/Native cinterop 定义指向 `include/media_engine.h` + `linkerOpts = -lmedia_engine -L<installPath>`. (2) 新 `bindings/kotlin-native/example/` — 最小 Gradle 项目 with `build.gradle.kts` + `nativeMain/kotlin/Main.kt` 调 `me_engine_create` / `me_engine_destroy` + print version。(3) Docs 在 `docs/INTEGRATION.md` 加"Kotlin/Native" section。Milestone §M7，Rubric §5.5。
- **jvm-jni-sample** — M7 exit criterion "JVM JNI 样例（macOS / Linux）". `grep -rn 'JNI\|jni\|javah' src include bindings 2>/dev/null` 空。**方向：** (1) 新 `bindings/jni/MediaEngine.java` — public Java class with native methods (engineCreate/Destroy/loadTimeline/renderStart/renderWait). (2) 新 `bindings/jni/me_jni.cpp` — JNI implementation bridging to C API + one example `main` Kotlin/Java runner。(3) JNI shared library build target in CMake (opt-in `ME_BUILD_JNI=ON`). Milestone §M7，Rubric §5.5。
- **async-job-base** — 当前只有 `me_render_start` 一个异步入口，worker→caller 的 error propagation 走 `Job::err_msg` + `me_render_wait` 中转。等第二个异步 API 落地（大概率 M6 frame-server async preview）时抽 `AsyncJobBase` 收口。Milestone §M7，Rubric §5.2。
- **codec-pool-real-pooling** — `src/resource/codec_pool.hpp:6` 注释: "encoder reuse (the "pool" in the name) is deferred"；`codec_pool.cpp` 只 `++live_count_` / `--live_count_`。`reencode-multi-clip` N-segment 开独立 AVCodecContext（`reencode_segment.cpp:112`），但每段 open_decoder 只 ~ms，没 profile 证据表明是瓶颈。**方向：** 等 benchmark 证实瓶颈再加 `get_or_make_decoder(codec_id, codecpar)` + `avcodec_flush_buffers` pool 路径。Milestone §M7-debt (cross)，Rubric §5.3。
...
- **vfr-drift-1h-bench** — vfr-av-sync (0d0094b) 的 helper + push_video_frame fix 把 VFR 源 PTS 保留到输出，已 unit-test (`test_reencode_pts_remap.cpp`, 117 assertions)。但"1 ms / 小时"阈值没有端到端 fixture 证据。**方向：** 新 `bench/bench_vfr_av_sync.cpp` — 生成 synthetic VFR input（libav API 直接构造 AVPacket with irregular pts），跑过 reencode，ffprobe output video+audio streams，比较最后帧的 video pts (scaled) vs audio pts (scaled)，assert |diff| / duration < 1e-6 (= 1 ms / 1000s = 1 ms / hour scaled down)。配置 `ME_BUILD_BENCH=ON`. Milestone §M7-debt (cross)，Rubric §5.2。
- **talevia-jvm-wrapper** — M7 exit criterion "在 talevia 内建 platform-impls/video-media-engine-jvm". 跨 repo 任务 — 需要 talevia 那边的工作，本 repo 内能做的就绪后标记完成。**方向：** 等 jvm-jni-sample 落地后，复制 JNI wrapper 到 talevia 目录 + build.gradle.kts + 替换 shell-out FFmpeg 的 passthrough 测试。本 repo cycle 只能准备 JNI wrapper；实际集成在 talevia 仓库。Milestone §M7，Rubric §5.5。
