...
- **me-render-frame-impl** — M6 exit criterion "`me_render_frame` 返回真实帧" 的起步。`src/api/render.cpp:99` 明标 `STUB: frame-server-impl — me_frame_* accessors; land with the M6 frame server`. `src/orchestrator/previewer.cpp:6` 有对应 `STUB: frame-server-impl — single-frame graph eval path; M6 milestone`。**方向：** (1) `me::orchestrator::Previewer::frame_at(t)` 实装——复用 compose_decode_loop 的 per-frame 逻辑，但只跑 ONE frame 到 RGBA buffer (不经 encoder)。(2) `me_render_frame` C API 实装：调 `Previewer::frame_at(engine, timeline, t)`, 返回 `me_frame_t` (typed struct: width, height, stride, RGBA8 data, caller-owned)。(3) test: load 2s timeline, request frame at t=1.0s, assert non-empty RGBA output + dimensions match。Milestone §M6，Rubric §5.2。
- **disk-cache-cache-dir** — M6 exit criterion "disk cache (`cache_dir` 配置生效)"。`grep -rn 'cache_dir' src include` 显示 `me_engine_config_t.cache_dir` 字段存在于 `include/media_engine/engine.h` 但 `grep 'mkdir\|filesystem\|cache_dir' src/resource` 空——没有 disk 实现。**方向：** 新 `src/resource/disk_cache.{hpp,cpp}` — 以 `cache_dir` 为根目录；key = frame content_hash，value = RGBA8 pixels + header (W, H, stride)。API: `put(hash, rgba, w, h, stride)` / `get(hash) -> optional<cached_frame>`。无 dir 或 dir 不可写 → 自动降级到 memory-only (no-op put)。Milestone §M6，Rubric §5.3。
