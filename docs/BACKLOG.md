...
- **test-reencode-multi-clip** — `reencode-multi-clip` 在 2bfa6cd 落地（60 h264 + 89 aac frames + 2.04s 端到端回归），但这条回归只手测一次没进 CI：`grep -n "h264.*aac.*2\|segment.*2" tests/test_determinism.cpp` 返回空。Refactor `process_segment` 时没 tripwire。**方向：** `tests/test_determinism.cpp`（或 `tests/test_reencode_multi_clip.cpp`）加一 case：两 clip 同 asset 的 timeline 走 h264/aac reencode，断言产物 ffprobe container=mov + 视频帧数 = 单 clip 2 倍 + byte-identical 跨两次 engine instance（bitexact flags 应保证）。Milestone §M1-debt，Rubric §5.3。
...
- **ocio-pipeline-wire-first-consumer** — `ocio-pipeline-factory` 在 3060977 落地，但 src/ 里**零 caller**：`grep -rn 'make_pipeline\|IdentityPipeline' src/` 只找到 `src/core/engine_seed.cpp:24` 的 future-extension 注释，没有实际调用。Factory 现在是纯自检的孤立代码。**方向：** Reencode pipeline 开 encoder 前 call `me::color::make_pipeline()` 存 `std::unique_ptr<Pipeline>` 到 `SharedEncState`，per-frame 调用 `apply(...)`（今天 IdentityPipeline 是 no-op，M2 OCIO 上线时切换 factory 返回值即消费端变化）。Milestone §M2-prep，Rubric §5.1。
...
- **debt-test-content-hash-seed-collisions** — `tests/test_content_hash.cpp:95-108` 覆盖 `seed` 正常路径和空 hash 路径，但**没测**"同一 URI seed 两次不同 hash 的 winning policy"。`src/resource/asset_hash_cache.cpp` 的 `seed(uri, hash)` 应该是 idempotent-per-URI（last-wins？first-wins？）——对 host 场景（用户更新 asset content hash）是真实可观察行为。**方向：** `tests/test_content_hash.cpp` 加一 case：`seed(uri, "aaa...")` → `seed(uri, "bbb...")` → `get_or_compute(uri)` 应该返回哪个？断言当前 impl 选的 policy，让 policy 变化必须显式改 test。Milestone §M1-debt，Rubric §5.2。
- **debt-iterate-gap-repopulate-grep-discipline** — 本 session 连续 3 个 cycle（`debt-render-bitexact-flags` / `me-timeline-loader-multi-track-reject` / `debt-test-cache-invalidate-coverage`）命中同一失误：repopulate 写 bullet 的 Gap 描述时没 grep 代码验证，结果 bullet 描述的"缺失"早已实装，cycle 都在"补边角"而非"做主工作"。**方向：** `.claude/skills/iterate-gap/SKILL.md` §R 加硬性要求："每条 bullet 的 Gap 必须引用具体 `path:line` 证据，不凭印象写'静默'/'没覆盖'等转述"；在 §R.5 或 §R 末尾加 repopulate 自检 checklist 4-5 条。Milestone §M1-debt，Rubric §5.2。
