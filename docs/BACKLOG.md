...
- **multi-track-compose-actual-composite** — M2 exit criterion "2+ video tracks 叠加, alpha / blend mode 正确" 的真正实装（最后一步）。`src/orchestrator/compose_sink.cpp:ComposeSink::process` 当前 bottom-only delegation。**两件 prereq 已就位**：`setup_h264_aac_encoder_mux`（`multi-track-compose-encoder-mux-helper` cycle）+ `pull_next_video_frame`（`multi-track-compose-frame-puller` cycle）。**phase-1 简化**：每 track 恰好 1 clip（去 within-track clip 切换复杂度）。剩余工作：ComposeSink::process 重写 body——(1) call `setup_h264_aac_encoder_mux(opts_built_from_demuxes_0, demuxes[0]->fmt, ...)` 拿 encoder + mux + SharedEncState；(2) `mux->open_avio` + `write_header`；(3) 对每 track 开独立 video AVCodecContext（via `me::resource::CodecPool`）；(4) per output frame at tl.frame_rate 驱动：`me::compose::active_clips_at(tl, T)` → for each TrackActive, `pull_next_video_frame(track.demux, track.vsi, track.dec, pkt, frame, err)`，`me::compose::frame_to_rgba8` 得 RGBA8 + `sws-scale` 到 target dims，`me::compose::alpha_over(dst, src, ..., 1.0, BlendMode::Normal)` bottom→top 叠加，`me::compose::rgba8_to_frame` 回 encoder YUV，`detail::encode_video_frame`；(5) audio：phase-1 仍从 `demuxes[0]` 读 packet passthrough（`multi-audio-mix` 另在 `audio-mix-scheduler-wire`）；(6) flush encoders + write trailer。新 `tests/test_compose_sink_e2e.cpp` 2-track e2e 测试——同源 fixture ×2 tracks → h264/aac → 产物非空 + 字节确定性。Milestone §M2，Rubric §5.1。
