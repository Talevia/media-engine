...
- **multi-track-compose-sink-wire** — M2 exit criterion "2+ video tracks 叠加, alpha + blend mode" 的实装承接。alpha-over 内核 + 3 blend mode 已在 `multi-track-compose-kernel` cycle 就位（`src/compose/alpha_over.{hpp,cpp}`，11 case / 37 assertion），但 `src/orchestrator/exporter.cpp:60` 的多轨 gate 仍然 `ME_E_UNSUPPORTED`——sink 层 / 多 demux 调度 / encoder 对接 RGBA 路径都没做。**方向：** 新 `ComposeSink`（或扩展 `H264AacSink` 支持多 demux input）：per-output-frame 从所有 active track 的对应源抽帧（基于 Timeline::clips track_id 分组 + timeline time → 每 track 选当前 active clip）→ sws-scale 每源到 RGBA8 → `me::compose::alpha_over` 逐 track 按 `Track::kind=Video` 顺序叠（tracks[0] 底层、tracks[N-1] 顶层）→ RGBA → YUV → encoder。翻 Exporter gate；e2e 测试 2-track timeline 产物字节确定性。Milestone §M2，Rubric §5.1。
