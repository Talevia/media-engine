...
- **debt-scan-debt-tests-coverage** — `tools/scan-debt.sh:28` 只扫 `src` / `include`（`find src include ...`）；测试文件不在 §1 长文件审计视野。测试目录实际有两个超 1000 行的巨型文件（`tests/test_timeline_schema.cpp` 1535 行、`tests/test_compose_sink_e2e.cpp` 1136 行），远超 src 的 400 行阈值，但 scan-debt 报 "0 long files"——漂白了真实债务。**方向：** scan-debt.sh §1 加第二个 `find tests` 段，用同样的 400 行阈值（或更宽容的 700 行，因为测试文件往往积累多个 TEST_CASE）。输出分 `## 1a. src/include long files` + `## 1b. tests long files`。Milestone §M7-debt (cross)，Rubric §5.3。
