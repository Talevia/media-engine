...
- **multi-track-compose-sink-wire** — M2 exit criterion "2+ video tracks 叠加, alpha + blend mode" 的最后实装。**四件前置已就位**：schema/IR（`multi-track-video-compose`）+ alpha_over 内核（`multi-track-compose-kernel`，`src/compose/alpha_over.{hpp,cpp}`）+ 多轨活动 clip 解析器（`multi-track-active-clips-resolver`，`src/compose/active_clips.{hpp,cpp}`）+ YUV↔RGBA8 转换 helper（`multi-track-compose-frame-convert`，`src/compose/frame_convert.{hpp,cpp}`）。仅剩把四者粘起来。`src/orchestrator/exporter.cpp:60` 的多轨 gate 仍 `ME_E_UNSUPPORTED`。**方向：** 新 `ComposeSink : public OutputSink`，持 N `DemuxContext`（每 track 一个）+ target 分辨率 + encoder handle；per output frame：`active_clips_at(tl, t)` → 每 TrackActive 从对应 demux 抽 source frame（现有 reencode_segment decode machinery 复用）→ `frame_to_rgba8` → 累积到中间 `dst_rgba` buffer → `alpha_over(dst_rgba, src_rgba, ..., opacity, BlendMode::Normal)` per track（bottom→top order）→ `rgba8_to_frame(dst_rgba, target_yuv_frame)` → send to encoder。翻 Exporter multi-track gate → `make_output_sink` 返回 ComposeSink。e2e 测试：2-track 同源 fixture（一 track 1s + 一 track 1s 同 demux 源）passthrough-unsuitable → h264 reencode，产出非空 + 两次 byte-identical。Milestone §M2，Rubric §5.1。
