...
- **hdr-color-space-schema-v2** — `src/timeline/timeline_loader.cpp` + `docs/TIMELINE_SCHEMA.md` 当前只接受 BT.709 / sRGB / linear；`grep -rn 'bt2020\|smpte2084\|arib-std-b67' src/ include/` 空。M10 HDR 端到端的 IR 起点：schema validation 需要识别 `colorSpace.{primaries=bt2020, transfer=smpte2084|arib-std-b67, matrix=bt2020nc, range=full|limited}` 且拒绝非法组合（如 `primaries=bt709 + transfer=smpte2084`）。**方向：** 扩 `me::timeline::parse_color_space` 接受新枚举值 + 新增 validate 表 + 单元测试覆盖合法 / 非法组合矩阵。后续 decode / encode / OCIO 路径才能 dispatch HDR 分支。Milestone §M10，Rubric §5.1。
