...
- **docs-ci-reference-clarity** — `docs/VISION.md:144` + `docs/TESTING.md:126,140,188` 引用 "CI hosts" / "Linux CI" 语境，但 `ls .github` 空、`find . -iname ".gitlab-ci.yml" -o -iname "ci*"` 空——本 repo 没 CI 配置。docs 含蓄假设 CI 存在。**方向：** 加 `docs/VISION.md` / `docs/TESTING.md` 顶部 note："This repo has no CI config; host repos (talevia) own cross-platform gating. All local ctest runs are dev-box only."，或者新 `.github/workflows/build-and-test.yml` 实现基本的 CI gate（macOS + Linux × `ME_WERROR=ON` ×`ctest`）。先落 doc clarifier；CI impl 单独 bullet。Milestone §M7-debt (cross)，Rubric §5.6。
- **examples-compose-scrub-text** — `ls examples/` 示范 01_passthrough / 02_graph_smoke / 03_timeline_segments / 04_probe / 05_reencode / 06_thumbnail。覆盖 M1 C API；M2-M6 feature（多轨 compose / me_render_frame scrubbing / text clip）的使用方式只能从 test code 反推。Host (talevia) 对接时只有试错渠道。**方向：** 新 `examples/07_compose_multitrack/main.c` — 2-track timeline.json + h264/aac 输出。`examples/08_frame_server_scrub/main.c` — me_render_frame 遍历 0–2s 输出 N 张 PNG + 打印 me_cache_stats。`examples/09_text_clip/main.c` — text+maxWidth timeline 渲染一张 PNG。Milestone §M7-debt (cross)，Rubric §5.6。
- **test-subtitle-renderer-codepage** — `src/text/subtitle_renderer.hpp:66` `load_from_memory(content, codepage)` 的 codepage 参数给 libass 处理 non-UTF-8 .srt。`grep -n 'codepage' tests/test_subtitle_renderer.cpp` 只命中结构体字段，没测 non-UTF-8 path。**方向：** 新 TEST_CASE — 用 libiconv 生成一份 cp1251 编码的 .srt 字节序列（或 hand-encode 一个 Кириллица 示例 `\xD1\xE5\xF0\xE3\xE5\xE9` = "Сергей"），verify `load_from_memory` 成功 + render_frame 输出像素非空。Milestone §M5-debt (cross)，Rubric §5.3。
- **debt-test-scratch-dir-helper** — `tests/test_disk_cache.cpp:42` + `tests/test_frame_server.cpp:65` 各自复制 `reinterpret_cast<uintptr_t>(this)` + `fs::create_directories` + RAII dtor-cleanup 的 scratch-dir pattern；`tests/test_frame_server_concurrent.cpp:78` 用 `reinterpret_cast<uintptr_t>(&fixture_path)` 因为它不是 struct 成员。三个拷贝、两种形态。**方向：** 新 `tests/scratch_dir.hpp` `class ScratchDir { fs::path path; ScratchDir(string_view test_slug); ~ScratchDir(); }` — 构造给 slug；析构 fs::remove_all。三处消费者切换到统一 helper。Milestone §M7-debt (cross)，Rubric §5.3。
