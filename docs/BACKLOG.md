...
- **cross-dissolve-transition-render-wire** — M2 exit criterion "Cross-dissolve transition" 的 render integration。四个 prereq 已就位：kernel (`src/compose/cross_dissolve.{hpp,cpp}`, 9 case / 28 assertion)、scheduler (`src/compose/active_clips.{hpp,cpp}` 的 `active_transition_at`, 3 case / 13 assertion)、`me::Clip::id`（`cross-dissolve-active-transition-scheduler` cycle 添加）、以及 **precedence resolver** (`src/compose/active_clips.{hpp,cpp}` 的 `frame_source_at` + `FrameSource` tagged struct, 4 case / 18 assertion，由 `cross-dissolve-frame-source-resolver` cycle 添加——把 "transition 窗口内 transition 优先" 规则封成纯函数并 resolve from/to clip_idx + source_time）。剩余工作：(1) render 路径改调 `frame_source_at(tl, ti, T)` 代替 `active_clips_at`（`src/orchestrator/compose_sink.cpp:231`）；(2) Transition kind 时从 from_clip decoder pull 一帧 at `transition_from_source_time`，从 to_clip decoder pull 一帧 at `transition_to_source_time`；(3) `cross_dissolve(out_rgba, from_rgba, to_rgba, W, H, stride, transition.t)` 混合；(4) `ComposeSink::process` / Exporter 的 transitions gate 翻（`src/orchestrator/exporter.cpp:90`）。需要每 track 支持 decoder 跨 clip 切换或同时持 2 decoder（复杂度来源）。e2e 测试：2-clip + 1s cross-dissolve timeline 渲染，产物比无 transition 版本不同。Milestone §M2，Rubric §5.1。
