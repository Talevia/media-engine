...
- **debt-split-bindings-jni-me-jni-cpp** — `wc -l bindings/jni/me_jni.cpp` = 335 lines + 每 cycle 仍在 +20-50 行（cycle 76 thumbnail +30, cycle 84 last_status +20, cycle 92 render_frame +50）。Trends toward §1a 400 ceiling within 2 cycles。**方向：** 抽 progress_trampoline + JavaCallback + JobWrap 到 sibling `me_jni_progress.{cpp,hpp}`；me_jni.cpp 留 JNI exports + JavaVM bookkeeping。Milestone §M7-debt (cross)，Rubric §5.3。
