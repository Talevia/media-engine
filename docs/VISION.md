...
- 任务要求 engine 做 AIGC 生成（文生图 / 文生视频 / 文生语音 / TTS / LLM 文本生成 / inpainting / super-resolution / colorization / style transfer 整帧重生成 / image-to-image diffusion 等"生成新内容"的能力）。**判别标准**：输出是 AI 新合成的像素 / 音频 / 文本 = AIGC，stop；输出是对输入的分析结果（坐标 / mask / 标签 / 文本识别 / 流场）= ML 推理，在边界内。
...
**media-engine 就是这个 Compiler 里「传统引擎 + 特效渲染 + ML 推理（非生成）」那一段。**

不做 AIGC 生成——文生图 / 视频 / 语音 / TTS / LLM 文本生成 / inpainting / super-resolution / colorization / style transfer 整帧重生成等"生成新内容"的能力都由 talevia 调外部服务。**做** ML 推理——人脸 landmark / 人体 keypoint / segmentation / depth / flow / object detection / scene understanding / OCR / speech recognition 等"对输入做分析、输出结构化结果"的能力，让 effect 和 timeline 能感知输入内容（贴纸跟随面部、自动美颜、蒙版抠像、姿态特效、自动字幕、智能裁剪等）。详细边界见 §3.5。
...
Engine 做 **ML 推理（不含生成式）**——把"对输入做分析、输出结构化结果"作为一等能力，让 timeline / effect 能感知输入内容。

**做**（推理 = 输出是输入的分析结果）：

- 空间结构：人脸 landmark（68/468 点）、人体 keypoint（COCO-17 / BlazePose 33）、人像 / 物体 segmentation mask、salience map、depth、optical flow
- 感知分类：object detection（含 bbox + 类别标签）、image classification / scene understanding、activity recognition、OCR
- 音频分析：speech recognition（语音转文本）、音频事件检测、说话人分离

**不做**（生成 = 输出是新合成的内容，参 §"发现不符" 判别标准）：

- AIGC 文 / 图 / 视频 / 音生成、LLM 文本续写、TTS 语音合成
- 整帧 / 整段重生成：style transfer、image-to-image diffusion、inpainting、outpainting、super-resolution、colorization、frame interpolation 中的 AI 生成新帧路径（线性插值的 retiming 不属于此列）
- 任何"凭少量输入产出大量新像素 / 新音频 / 新文本"的能力

判别标准：**输出是 AI 新合成的像素 / 音频 / 文本** = AIGC，stop；**输出是对输入的分析结果**（坐标 / mask / 标签 / 文本识别 / 流场 / 类别概率）= ML 推理，在边界内。

设计意图：让 effect / timeline 的"内容感知"成为 engine 一等能力（贴纸跟随面部、智能裁剪追主体、自动字幕、活动检测分段、OCR 文字编辑），同时把"生成式"留给 talevia 上层 + 外部服务，避免 engine 滑成"什么 AI 都干"的 SDK。生成式与推理的工程取舍非常不同——生成式的输入空间小、输出空间大、prompt 工程占主导、模型尺寸 GB 级、license 多被污染；推理的输入空间大（每帧）、输出空间小、确定性 + 缓存 + license 白名单都跟引擎现有纪律对齐。

工程要求（与 ML 范围无关，是工程纪律）：

- **推理结果走 §3.3 contentHash 缓存**：`(model_id, model_version, 量化 mode, 输入帧 hash)` 作为 key；同样的输入跨 timeline / 跨调用复用。
- **模型权重不内置 binary**：通过 timeline asset 或 host 注入提供；engine 校验 content_hash 而不是信任路径。Apache / MIT / BSD / CC-BY 等许商用模型走 lazy fetch（host 实现 fetcher callback）。
...
- **timeline IR 显式声明**：推理 asset 必须 pin model_id + version + 量化 mode；缓存键含这三项；模型升级 = 新 content_hash = 缓存自动失效。
...
- 中间结果可失效、可观测的缓存
- 通过 C API 被任意宿主驱动
- ML 推理（人脸 landmark / 人体 keypoint / segmentation / depth / flow / object detection / scene understanding / OCR / speech recognition 等"分析输入产出结构化结果"的能力）——参 §3.5 边界条件（**不含生成式**），**可选模块**（CMake `ME_WITH_INFERENCE=ON`）
...
- AIGC 生成——文生图 / 文生视频 / 文生语音 / TTS / LLM 文本生成 / inpainting / outpainting / super-resolution / colorization / style transfer 整帧重生成 / 任何"生成新内容"的 AI 能力（参 §3.5 判别标准）——talevia 调外部服务。**ML 推理（分析）在 §3.5 边界内做**，包括 landmark / segmentation / classification / detection / OCR / speech recognition 等"输出是输入分析结果"的能力
...
新增 ML 能力或 ML-driven effect 时按此自查：

- **AIGC 判别**：输出是 AI 新合成的像素 / 音频 / 文本吗？是 = AIGC，stop（参 §3.5 不做列表）。输出是对输入的分析结果（坐标 / mask / 标签 / 流场 / 文本识别 / 概率分布）= 推理，继续往下。
...
- 推理 runtime 在白名单（CoreML / ONNX runtime / TFLite）吗？CUDA library / MKL / 闭源 SDK = stop。
