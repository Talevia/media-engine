...
9. **Five-module roles stay unmixed** (see `docs/ARCHITECTURE_GRAPH.md`):
    ...
    - `orchestrator/` = Player / Exporter (classes) + path (a) free functions in `compose_frame.{hpp,cpp}` — hold Timeline, compile per-segment Graphs; they do NOT own Nodes, do NOT define kernels, and they are NOT editors (interactive editing is host-side). Three execution models: (a) per-frame stateless = free functions `resolve_active_clip_at` / `compile_frame_graph` / `compose_frame_at` / `compose_png_at`, called by `me_render_frame` + `Player::producer_loop` + composition thumbnail; (b) streaming stateful = Exporter; (c) playback session = Player composes (a)+(b)+ internal clock. Player **承担**播放会话（时钟、状态机、A/V sync）but **不画 UI surface**——窗口 / widget / 绘制目标 still host-side. The asset-level `me_thumbnail_png` C API is a separate path that lives in `src/api/thumbnail.cpp` and does NOT go through `compose_png_at` (asset URI vs Timeline are different inputs).
